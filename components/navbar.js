...
export default class NavBar extends React.Component {
  ...
  async navbarComponents() {
    if(this.state.username !== (parseCookies().username || '')) {
      this.setState({username:parseCookies().username || ''});
      this.setState({token: parseCookies() || ''});
    }
    try {
      var navItems;
      if (this.state.token !== "") {
        ...
        <li id={styles.userTab} className={styles.navDrop + " nav-item dropdown"}>
          ...
          <img className="d-inline-block align-self=center" style={{minWidth: "15px", width:"1.5rem", paddingRight:"10px"}} src="/user.png/"></img>
            {this.state.username}
          </a>
        ...
        <li id={styles.userAdd} className={styles.navDrop + " nav-item dropdown"}>
            ...
            <div className={styles.nav +" dropdown-menu"} aria-labelledby="navbarDropdownMenuLink">
              ...
              <Link href="/addServer"><a className={styles.navItem + " dropdown-item"} href="#">Add Server</a></Link>
            </div>
          </li>
      </ul>);
      }
    ...
    return(
      <div>
        ...
        <nav style={{backgroundColor:"#2c2c54"}} className="navbar navbar-expand-lg navbar-dark">
          <Link href="/"><img className="d-inline-block align-top" style={{width:"2rem"}} src="/logo.png/"></img></Link>
